...
/**
 * Abstract Test class for Injections.
 *
...
public abstract class AbstractInjectionTest
   ...
   */
  public AbstractInjectionTest(String name) {
    super(name);
  ...
  protected CheckOptionHandler getOptionTester() {
    ...
    if (getInjection() instanceof OptionHandler)
      result.setOptionHandler((OptionHandler) getInjection());
    else
   ...
   * Sets the Injection returned from the getInjection() method.
   * 
  ...
  /**
   * Simple method to return the injected set of test instances after
   * passing through the test algorithm. m_Injection contains the algorithm
  ...
  protected Instances useInjection() {
    ...
    try {
      result = m_Injection.buildInjection(icopy);
    ...
    try {
      result = m_Injection.inject(icopy);
      assertNotNull(result);
    }
    catch (Exception ex) {
      ex.printStackTrace();
  ...
  public void testSerialVersionUID() {
    if (SerializationHelper.needsUID(m_Injection.getClass()))
  ...
  public void testChangesInputData() {
    ...
    // 1st imputation
    try {
      m_Injection.buildInjection(m_Instances);
      result = m_Injection.inject(m_Instances);
